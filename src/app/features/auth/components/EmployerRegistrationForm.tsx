...
export default function EmployerRegistrationForm({
  ...
  const step2Form = useForm<EmployerStep2Data>({
  ...
  const handleStep2Submit = async (data: EmployerStep2Data) => {
    if (!step1Data) return;
    ...
    try {
      const completeData: EmployerRegistrationData = {
        ...step1Data,
        ...data,
      ...
      if (result?.error) {
        console.error(
          '[REGISTRATION] Employer registration failed:',
          result.error
        );
      ...
      if (result?.success) {
        console.log('[REGISTRATION] Employer registration successful');
      } else {
        setIsPending(false);
      }
    } catch (error: unknown) {
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-blue-700 mt-1">
                  {`We'll help you fill in company details in the next step`}
                </p>
              </div>
            </div>
          </div>
